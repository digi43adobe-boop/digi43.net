...
const SEED_POSTS: Post[] = [
  ...
  },
  {
    ...
    content: {
      ...
      en: {
        title: "Adobe Creative Cloud for Teams vs Enterprise: how to choose",
        excerpt:
...
export async function seedIfEmpty(): Promise<{ inserted: number }> {
  const rows = (await sql`SELECT COUNT(*)::int AS c FROM posts`) as { c: number }[];
  if (rows[0]?.c > 0) return { inserted: 0 };
  for (const p of SEED_POSTS) await upsertPost(p);
  return { inserted: SEED_POSTS.length };
}
